...
(function(root, factory) {
  ...
  lunar.removeClass = function(elem, name) {
    var remove = elem
      .attr("class")
      .replace(new RegExp("(\\s|^)" + name + "(\\s|$)", "g"), "$2");
...
(function($) {
    ...
    init: function(options) {
      ...
      $.each(this, function(a, b) {
        ...
        template = template.replace(
          'width="34"',
          'width="' + audio.data("size") + '"'
        );
        template = template.replace(
          'height="34"',
          'height="' + audio.data("size") + '"'
        );
      ...
      $(_.defaults.audioObj).on("progress", function(e) {
        if (this.buffered.length > 0) {
          ...
          var cache = $(e.currentTarget)
              .parent()
              .find(_.settings.precache),
  ...
  $.fn.mediaPlayer = function(methodOrOptions) {
    if (_[methodOrOptions]) {
      return _[methodOrOptions].apply(
        this,
      ...
      $.error(
        "Method " + methodOrOptions + " does not exist on jQuery.mediaPlayer"
      );
    }
  };
})(jQuery);
